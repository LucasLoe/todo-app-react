...
export function TaskList({ itemCSS, deleteTask, completeTask, tasks }) {
  return (
    <div className="w-full mx-auto bg-white rounded-lg shadow-lg max-w-xl box-border">
      <ul className="divide-y-2 divide-gray-100 max-w-xl">
        {tasks.map((e) => (
          <li
            key={e.uuid}
            className={
              e.status === "pending" ? itemCSS.cssPending : itemCSS.cssCompleted
            }
          >
            <input
              type="checkbox"
